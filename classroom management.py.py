...
def Database():
    ...
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS STUD_REGISTRATION (STU_ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, STU_NAME TEXT, STU_CONTACT TEXT, STU_EMAIL TEXT, STU_ROLLNO TEXT, STU_BRANCH TEXT,STU_MARKS INTEGER)")
...
def DisplayForm():
    ...
    display_screen.geometry("1100x550")
    #setting title for window
    display_screen.title("CLASSROOM MANAGEMENT SYSTEM")
    global tree
    global SEARCH
    global name,contact,email,rollno,branch,marks
    SEARCH = StringVar()
    name = StringVar()
    contact = StringVar()
    email = StringVar()
    ...
    #label for heading
    lbl_text = Label(TopViewForm, text="CLASSROOM MANAGEMENT SYSTEM", font=('verdana', 18), width=600,bg="#1C2833",fg="white")
    ...
    Label(LFrom, text="Marks ", font=("Arial", 12)).pack(side=TOP)
    Entry(LFrom, font=("Arial", 10, "bold"), textvariable=marks).pack(side=TOP, padx=10, fill=X)
    ...
    btn_marks = Button(LeftViewForm, text="Sort", command=arrange_marks)
    btn_marks.pack(side=TOP, padx=10, pady=10, fill=X)
   ...
   #setting scrollbar
    ...
    tree = ttk.Treeview(MidViewForm,columns=("Student Id", "Name", "Contact", "Email","Rollno","Branch","Marks"),
    ...
    tree.heading('Marks', text="Marks", anchor=W)
...
def register():
    ...
    if name1 == '' or con1 == '' or email1 == '' or rol1 == '' or branch1 == '' or marks1 == '' :
        tkMessageBox.showinfo("Warning", "fill the empty field!!!")

 #   elif rol1!='' :
 #    roll_no_validate()

    elif  marks1.isnumeric() == False :
         tkMessageBox.showinfo("Warning", "Enter Proper marks")

    else:
        # execute query
        conn.execute('INSERT INTO STUD_REGISTRATION (STU_NAME,STU_CONTACT,STU_EMAIL,STU_ROLLNO,STU_BRANCH,STU_MARKS) \
                  VALUES (?,?,?,?,?,?)', (name1, con1, email1, rol1, branch1, marks1));
        conn.commit()
...
def roll_no_validate() :
    Database()
    rol1 = rollno.get()
    cur = conn.cursor()
    conn.execute("select STU_ROLLNO from STUD_REGISTRATION where STU_ROLLNO=?", [rol1])
  #  count =cur.rowcount
    data = cur.fetchone()
    if not data:
        print('OK to insert')
    else:
        tkMessageBox.showinfo("Warning", "Roll no already exist!!!")
...
def arrange_marks():
    Database()
    tree.delete(*tree.get_children())
    cursor = conn.execute("SELECT * FROM STUD_REGISTRATION  order by STU_MARKS desc")
    fetch = cursor.fetchall()
    for data in fetch:
        tree.insert('', 'end', values=(data))
    cursor.close()
